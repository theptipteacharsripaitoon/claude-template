...
- **Invisible-Unicode cleanup for Thai data.** Strip zero-width and invisible characters BEFORE trim/compare/join on Thai text — at minimum: ZWSP (U+200B), ZWNJ (U+200C), ZWJ (U+200D), BOM/ZWNBSP (U+FEFF), NBSP (U+00A0). Two visually identical strings that refuse to join means this. Keep Thai text `DT_WSTR` (Unicode) end to end — a `DT_STR` codepage round-trip silently destroys characters.
